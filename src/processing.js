...
function rewritePlaceholders(fnName, fnStr, asyncVarList) {
    ...
    while ((match = fnStr.match(Constants.ASYNC_PLACEHOLDER_REGEXP))) {
        ...
        // If the varname === $ then it's the anonymous placeholder: we need to wrap a special function around the
        // callback to mark it as anonymous.
        if (varName !== "$") {
            // Add the matched variable (without brackets) to a definition list
            _addToArray(asyncVarList, varName);
...
/**
 * Adds item to array if it's not already present
 *
